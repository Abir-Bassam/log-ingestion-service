...
- **Removed the trigram index.** Removing it raised my ingestion from 8,432 to over
  ...
  searches much slower (about 390 ms instead of 7 ms), but since ingestion is the main job
...
- **Text search (`q`) doesn't have a trigram index**, so it needs other filters (like time
  ...
  takes about 390 ms instead of 7 ms. I measured both GIN and GIST versions of the index
